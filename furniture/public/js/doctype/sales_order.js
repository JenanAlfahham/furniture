...
frappe.provide("furniture");
frappe.provide("furniture.selling");

furniture.selling.CustomSalesOrderController = class CustomSalesOrderController extends erpnext.selling.SalesOrderController {
    refresh(doc, dt, dn){
        var me = this;
    ...
    make_project() {
		frappe.model.open_mapped_doc({
			method: "furniture.www.api.make_project",
			frm: this.frm
		})
	}
    create_clearence(){
        frappe.model.open_mapped_doc({
			method: "furniture.www.api.create_clearence",
			frm: this.frm
		})
    }
    create_costing_note(){
        frappe.model.open_mapped_doc({
			method: "furniture.www.api.create_costing_note",
			frm: this.frm,
            args: {
                doctype: "Sales Order"
            }
...
extend_cscript(cur_frm.cscript, new furniture.selling.CustomSalesOrderController({frm: cur_frm}));
