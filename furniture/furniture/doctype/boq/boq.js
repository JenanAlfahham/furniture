...
frappe.ui.form.on('BOQ', {
	...
	boq_template: function(frm){
		if (frm.doc.item && frm.doc.boq_template){
			frappe.call({
				"method": "furniture.furniture.doctype.boq.boq.set_boq_template",
				"args": {
...
frappe.ui.form.on('Material costs', {
	...
	last_purchase_rate: function(frm, cdt, cdn){
		let row = locals[cdt][cdn];
		frappe.model.set_value(cdt, cdn, "cost", row.last_purchase_rate);
	},
	...
	uom: function(frm, cdt, cdn){
		let row = locals[cdt][cdn];
		set_conversion_factor(frm, row);
	},
	conversion_factor: function(frm, cdt, cdn){
		let row = locals[cdt][cdn];
		frappe.model.set_value(cdt, cdn, "cost", row.conversion_factor * row.last_purchase_rate);
	}
});
...
const set_conversion_factor = function(frm, row){
	...
	frappe.call({
		...
		callback: function(r){
			if (r.message){
				frappe.model.set_value(row.doctype, row.name, "conversion_factor", r.message.conversion_factor);
			}
		}
	})
}
